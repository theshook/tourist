...
exports.towns_get_all = (req, res) => {
  ...
  db.query('SELECT count(town_no) as total FROM towns WHERE town_inactive=0 AND town_delete=0', (err, total_items) => {
    if(err){
      throw err;
    }
    ...
    if(err){
      ...
      res.render('Admin/template', {user: req.user, total_pages: total_pages, rows: rows, pageTitle: 'Town Panel', page: 'Town/Town'});
    });
  });
}

exports.towns_new = (req, res) => {
  res.render('Admin/template', {user: req.user, pageTitle: 'Town Panel', page: 'Town/New'});
}
...
exports.towns_create = (req, res) => {
  ...
  db.query('INSERT INTO towns(town_name, town_zipcode, town_iscity, town_encode, town_encode_date) values(?,?,?,?,?)', 
...
exports.towns_edit = (req, res) => {
  ...
  (err, row) => {
    if (err) {
      ...
      res.render('Admin/template', {user: req.user, row: row, pageTitle: 'Town Panel', page: 'Town/Update'});
    }
  });
}
...
exports.towns_update = (req, res) => {
  ...
  db.query('UPDATE towns SET town_name=?, town_zipcode=?, town_iscity=?, town_encode=?, town_encode_date=? WHERE town_no=?',
...
exports.towns_delete = (req, res) => {
  date = moment().format("YYYY-MM-DD HH:MM:SS");
  let user = req.user.user_no;
  db.query('UPDATE towns SET town_delete=1, town_encode=?, town_encode_date=? where town_no=?', [user, date, req.params.townId], (err, row) => {
    if (err) {
      throw err;
    } else {
      res.redirect('/admin/town');
