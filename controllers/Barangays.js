...
exports.barangays_get_all = (req, res) => {
  ...
  db.query(
    "SELECT count(bar_no) as total FROM barangays",
    (err, total_items) => {
      if (err) {
        throw err;
      }
    ...
    LIMIT ?, ?`;
      ...
      db.query(query, data, (err, rows) => {
        ...
        res.render("Admin/template", {
          ...
          rows: rows,
          pageTitle: "Barangay Panel",
          page: "Barangay/Barangay"
        });
      });
    }
  );
...
exports.barangays_new = (req, res) => {
  db.query(
    "SELECT town_no, town_name FROM towns WHERE town_inactive=0 AND town_delete=0 order by town_name asc",
    (err, rows) => {
      if (err) {
        throw err;
      }
...
exports.barangays_edit = (req, res) => {
  ...
  db.query(
    "SELECT barangays.town_no, bar_no, bar_name, town_name FROM barangays INNER JOIN towns ON barangays.town_no = towns.town_no where bar_no = ?",
    [id],
    (err, row) => {
      db.query(
        "SELECT town_no, town_name FROM towns order by town_name asc",
        (err, data) => {
          if (err) {
            throw err;
          } else {
