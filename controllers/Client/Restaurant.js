...
exports.get_all_Restaurant = (req, res) => {
  db.query(
    ...
    WHERE (encode_delete=0 AND encode_inactive=0) AND establistments.ec_no=1 AND establistments_photo.image_isprimary=1
...
exports.restaurant_View = (req, res) => {
  ...
  WHERE establistments.estab_no = ?`, [id], (err, rows) => {
      ...
      res.render('Client/Restaurant/view', { rows: rows, pageTitle: 'Restaurant Information' });
    });
};
